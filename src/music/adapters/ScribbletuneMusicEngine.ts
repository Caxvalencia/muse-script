...
interface ScribbleApi {
  ...
  arp(value: string | { chords: string; count?: number; order?: string }): string[];
...
export class ScribbletuneMusicEngine implements MusicEngine {
  ...
  async init() {
    ...
    if (!this.scribble) this.scribble = await import("scribbletune") as unknown as ScribbleApi;
  }
  ...
  async load(song: CompiledSong) {
    ...
    for (const channel of resolved.channels) {
      for (const clip of channel.clips) {
        clip.notes = clip.notes.flatMap((note) => typeof note === "string" ? this.resolveClipTheory(note) : [note]);
        clip.events?.forEach((event) => { event.notes = event.notes.flatMap((note) => this.resolveEventTheory(note)); });
      }
    }
    await this.fallback.load(resolved);
  }

  play() { return this.fallback.play(); }
  stop() { this.fallback.stop(); }
  restart() { return this.fallback.restart(); }
  setTempo(bpm: number) { this.fallback.setTempo(bpm); }
  dispose() { this.fallback.dispose(); }
  ...
  private resolveClipTheory(value: string): Array<string | string[]> {
    ...
    if (value.startsWith("@progression:")) {
      ...
      return chords.split(" ").map((chord) => this.scribble!.chord(normalizeProgressionChord(chord)));
    }
    if (value.startsWith("@arp:")) {
      const [chords, count, order] = value.slice(5).split("|");
      return count ? this.scribble.arp({ chords, count: Number(count), order }) : this.scribble.arp(chords);
    }
    return [value];
  }
